  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (res.data.role !== "user") {
        setError("Access Denied: Please use the correct login page for your role.");
        setLoading(false);
        return;
      }
    ...
    <div className="flex items-center justify-center min-h-screen w-full" style={{ background: "#e0e5ec" }}>
      <div className="clay p-10 w-96">
        ...
        <button
          ...
          style={{ background: loading ? "#86efac" : "#16a34a", cursor: loading ? "not-allowed" : "pointer" }}
          onClick={handleLogin}
          disabled={loading}
        >
        ...
        <div className="mt-4 text-center text-sm text-gray-400 font-medium">
          New here?{" "}
          <a href="/register" className="font-bold text-green-600 hover:underline">
            Create Account
          </a>
        </div>
